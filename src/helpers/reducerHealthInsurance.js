...
export default function healthInsuranceReducer(state = initialState, action) {
    ...
    switch (action.type) {
        case "SET_SALARY": {
            ...
            const healthInsurance = calculateHealthInsurance(salary, isMember, year);
            return {
                ...state,
                salary: salary,
        ...
        case "SET_HEALTH_INSURANCE_MEMBER": {
            ...
            const isMember = action.payload?.isMember !== undefined ? action.payload.isMember : false;
            const healthInsurance = calculateHealthInsurance(salary, isMember, year);
            return {
                ...state,
                isMember: isMember,
...
export const selectIsHealthInsuranceMember = (state) => {
    const value = state.healthInsurance?.isMember === true;
    return value;
};